...
*/
bool dos_attack( lista_t* ip_list ){
	
	// si esta vacio o el largo de tiempos es menor a 5 no hay atacke
	if ( lista_esta_vacia(ip_list) || lista_largo(ip_list) < 5 ) return false;
	
	lista_iter_t* primero = lista_iter_crear(ip_list);
	
	if (!primero) return false;
	
	lista_iter_t* ultimo = lista_iter_crear(ip_list);
	...
	for ( int i = O; i <= lista_largo(ip_list) - 5; i++ ){
