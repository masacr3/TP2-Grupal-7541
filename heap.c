...
void downheap(void** arreglo,size_t cantidad,size_t posicion_padre,cmp_func_t cmp){
  ...
  size_t poscion_minimo=posicion_padre;
  if (posicion_hijo_derecho>=cantidad || posicion_hijo_izquierdo>=cantidad){
    ...
    if (posicion_hijo_derecho>=cantidad && comparar(arreglo,cmp,posicion_padre,posicion_hijo_izquierdo)>0) poscion_minimo=posicion_hijo_izquierdo;
    if (posicion_hijo_izquierdo>=cantidad && comparar(arreglo,cmp,posicion_padre,posicion_hijo_derecho)>0) poscion_minimo=posicion_hijo_derecho;
  }
  else{
    if (comparar(arreglo,cmp,posicion_padre,posicion_hijo_izquierdo)>0 && comparar(arreglo,cmp,posicion_hijo_izquierdo,posicion_hijo_derecho)<0) poscion_minimo=posicion_hijo_izquierdo;
    if (comparar(arreglo,cmp,posicion_padre,posicion_hijo_derecho)>0 && comparar(arreglo,cmp,posicion_hijo_derecho,posicion_hijo_izquierdo)<0) poscion_minimo=posicion_hijo_derecho;
  }
  if (posicion_padre==poscion_minimo) return;
  swap(&arreglo[posicion_padre],&arreglo[poscion_minimo]);
  downheap(arreglo,cantidad,poscion_minimo,cmp);
}
...
void* heap_ver_min(const heap_t* heap){
